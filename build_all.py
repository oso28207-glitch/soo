...
.player-shell{background:#000;border-radius:var(--r);overflow:hidden;margin-bottom:1.5rem;aspect-ratio:16/9}
...
var key="tf_p_"+window.location.pathname;var s=parseFloat(localStorage.getItem(key)||"0");
...
def base(title, body, depth=0, head="", scripts=""):
    ...
    return (
        ...
        + head + '\n</head>\n<body>\n'
        f'<header class="topbar"><a href="{p}index.html" class="logo">Telegram<span>Flix</span></a>'
        f'<nav class="nav"><a href="{p}index.html">الرئيسية</a></nav></header>\n'
...
def render_index(series):
    if not series:
        body = ('<section class="hero"><h1>المكتبة فارغة</h1></section>'
                '<div class="empty"><p>أضف بيانات إلى <code>data.json</code></p></div>')
    ...
    for s in series:
        ...
        if not poster:
            for eps in s["seasons"].values():
                for e in eps:
                    if e.get("thumb_url"):
                        poster = e["thumb_url"]; break
                if poster: break
        thumb = (f'<img loading="lazy" src="{esc(poster)}" alt="{esc(s["name"])}">'
                 if poster else f'<div class="no-thumb">{esc(s["name"][0])}</div>')
        cards.append(
            f'<a class="card" href="{url}">'
            f'<div class="card-thumb">{thumb}'
            f'<div class="card-overlay"><span class="badge">{s["total_episodes"]} حلقة</span></div>'
            f'</div><div class="card-body"><h3>{esc(s["name"])}</h3>'
            f'<p>{s["season_count"]} موسم</p></div></a>'
        )
    body = (f'<section class="hero"><h1>مكتبة المسلسلات</h1>'
            f'<p>{len(series)} مسلسل</p></section>'
        ...
        for eps in s["seasons"].values():
            for e in eps:
                if e.get("thumb_url"):
                    poster = e["thumb_url"]; break
            if poster: break
    poster_html = f'<img src="{esc(poster)}" alt="{esc(s["name"])}">' if poster else ""
    start = ""
    keys = list(s["seasons"].keys())
    if keys and s["seasons"][keys[0]]:
        first = s["seasons"][keys[0]][0]
        start = f'<a class="btn primary" href="../watch/{first["message_id"]}.html">▶ ابدأ المشاهدة</a>'
    sh = []
    for sk, eps in s["seasons"].items():
        ec = []
        for e in eps:
            thumb = (f'<img loading="lazy" src="{esc(e["thumb_url"])}" alt="حلقة {e["episode"]}">'
                     if e.get("thumb_url") else "")
            ec.append(
                f'<a class="episode-card" href="../watch/{e["message_id"]}.html">'
                f'<div class="episode-thumb">{thumb}<span class="play-icon">▶</span></div>'
                f'<div class="episode-info"><h4>الحلقة {e["episode"]}</h4>'
                f'<span class="duration">{dur(e.get("duration"))}</span></div></a>'
            )
        sh.append(f'<section class="season"><h2>الموسم {sk}</h2>'
                  f'<div class="episodes">{"".join(ec)}</div></section>')
    body = (
        f'<section class="series-header"><div class="series-poster">{poster_html}</div>'
        f'<div class="series-meta"><h1>{esc(s["name"])}</h1>'
        f'<p>{s["season_count"]} موسم · {s["total_episodes"]} حلقة</p>{start}</div></section>'
        + "".join(sh)
    )
...
def render_watch(name, season, episode, prev_ep, next_ep, ep):
    ...
    if video_url:
        player = (f'<video id="player" playsinline controls preload="metadata"{poster_attr}>'
                  f'<source src="{esc(video_url)}" type="video/mp4"></video>')
    else:
        tg = ep.get("telegram_url", "")
        fb = (f'<a href="{esc(tg)}" target="_blank" rel="noopener">شاهده على تليجرام</a>'
              if tg else "لا يوجد رابط متاح.")
        player = f'<div class="no-player"><p>⚠️ الفيديو غير متاح. {fb}</p></div>'
    ...
    body = (
        ...
        f'<div class="watch-info"><h1>{esc(name)}</h1>'
        f'<h2>الموسم {season} · الحلقة {episode}</h2>'
        f'<div class="watch-nav">{prev_btn}'
        f'<a class="btn" href="{series_url}">📺 كل الحلقات</a>{next_btn}</div>'
        f'<div class="autoplay-bar"><label>'
        f'<input type="checkbox" id="autoplay-toggle" checked> تشغيل التالية تلقائياً</label>'
        f'<span id="countdown"></span></div></div></div>'
        f'<script>window.__NEXT_URL__ = {next_json};</script>'
    )
    head = '<link rel="stylesheet" href="https://cdn.plyr.io/3.7.8/plyr.css">'
    scripts = ('<script src="https://cdn.plyr.io/3.7.8/plyr.polyfilled.js"></script>'
...
def build(clean=False):
    ...
    (DOCS / "404.html").write_text(
        base("404", '<section class="hero"><h1>404</h1>'
                    '<p><a href="index.html">الرئيسية</a></p></section>'),
        encoding="utf-8")
    ...
    for s in series:
        ...
        for sk, eps in s["seasons"].items():
            for i, ep in enumerate(eps):
                ...
                html_out = render_watch(s["name"], int(sk), ep["episode"], prev_ep, next_ep, ep)
